...
class StdlibDeploymentTarget(object):
    OSX = DarwinPlatform("macosx", archs=["arm64"],
                         sdk_name="OSX")

    iOS = DarwinPlatform("iphoneos", archs=["arm64", "arm64e"],
                         sdk_name="IOS")
    iOSSimulator = DarwinPlatform("iphonesimulator", archs=["arm64"],
                                  sdk_name="IOS_SIMULATOR",
                                  is_simulator=True)

    AppleTV = DarwinPlatform("appletvos", archs=["arm64"],
                             sdk_name="TVOS")
    AppleTVSimulator = DarwinPlatform("appletvsimulator", archs=["arm64"],
    ...
    FreeBSD = Platform("freebsd", archs=["aarch64"])

    LinuxStatic = Platform('linux-static', sdk_name='LINUX_STATIC', archs=[
        'aarch64'])

    OpenBSD = OpenBSDPlatform("openbsd", archs=["aarch64"])

    Cygwin = Platform("cygwin", archs=["arm64"])

    Android = AndroidPlatform("android", archs=["armv7", "aarch64"])

    Windows = Platform("windows", archs=["arm64"])

    Haiku = Platform("haiku", archs=["arm64"])
